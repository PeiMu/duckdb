...
ifeq (${ENABLE_QUERY_SPLIT}, 1)
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_QUERY_SPLIT=1
else
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_QUERY_SPLIT=0
endif

ifeq (${ENABLE_PARALLEL_EXECUTION}, 1)
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_PARALLEL_EXECUTION=1
else
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_PARALLEL_EXECUTION=0
endif

ifeq (${ENABLE_DEBUG_PRINT}, 1)
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_DEBUG_PRINT=1
else
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_DEBUG_PRINT=0
endif

ifeq (${ENABLE_CROSS_PRODUCT_REWRITE}, 1)
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_CROSS_PRODUCT_REWRITE=1
else
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_CROSS_PRODUCT_REWRITE=0
endif

ifeq (${ENABLE_MERGE_BACK_PLAN}, 1)
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_MERGE_BACK_PLAN=1
else
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_MERGE_BACK_PLAN=0
endif

ifeq (${ENABLE_SPECIFY_EST_STAT}, 1)
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_SPECIFY_EST_STAT=1
else
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_SPECIFY_EST_STAT=0
endif

ifeq (${ENABLE_MEASURE_EXE_TIME}, 1)
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_MEASURE_EXE_TIME=1
else
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_MEASURE_EXE_TIME=0
endif

ifeq (${ENABLE_CONVERT_IR_TO_DUCKDB}, 1)
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_CONVERT_IR_TO_DUCKDB=1
else
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_CONVERT_IR_TO_DUCKDB=0
endif

ifeq (${ENABLE_CONVERT_DUCKDB_TO_IR}, 1)
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_CONVERT_DUCKDB_TO_IR=1
else
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_CONVERT_DUCKDB_TO_IR=0
endif

ifeq (${ENABLE_CONVERT_IR_TO_SQL}, 1)
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_CONVERT_IR_TO_SQL=1
else
	CMAKE_VARS:=${CMAKE_VARS} -DDEFINE_CONVERT_IR_TO_SQL=0
endif
