...
mv ${LOG_NAME} official_${LOG_NAME}

###### join order opt + split + merge back to the whole plan
cd ../ && make clean && GEN=ninja ENABLE_QUERY_SPLIT=1 ENABLE_MERGE_BACK_PLAN=1 ENABLE_CROSS_PRODUCT_REWRITE=0 VERBOSE=1 ENABLE_MEASURE_EXE_TIME=1 make && cd measure/
...
mv ${LOG_NAME} js_whole_${LOG_NAME}

###### reorder table + split + join order opt + merge back to the whole plan
cd ../ && make clean && GEN=ninja ENABLE_QUERY_SPLIT=1 ENABLE_MERGE_BACK_PLAN=1 ENABLE_CROSS_PRODUCT_REWRITE=1 VERBOSE=1 ENABLE_MEASURE_EXE_TIME=1 make && cd measure/
...
mv ${LOG_NAME} rsj_whole_${LOG_NAME}
