...
echo "query_split with join_order_optimization after query_split, with estimated stats" > -a compile.log
...
echo "query_split with join_order_optimization after query_split" > -a compile.log
cd ../ && make clean && GEN=ninja ENABLE_QUERY_SPLIT=1 ENABLE_CROSS_PRODUCT_REWRITE=1 VERBOSE=1 make 2>&1|tee -a compile.log && cd measure && bash ./hyperfine_in_mem_job.sh query_split rsj
