...
cd ../ && make clean && GEN=ninja ENABLE_QUERY_SPLIT=0 VERBOSE=1 make > -a compile.log && cd measure && bash ./hyperfine_in_mem_job.sh official nan
...
cd ../ && make clean && GEN=ninja ENABLE_QUERY_SPLIT=1 ENABLE_CROSS_PRODUCT_REWRITE=1 ENABLE_SPECIFY_EST_STAT=1 VERBOSE=1 make > -a compile.log && cd measure && bash ./hyperfine_in_mem_job.sh query_split rsj_wo_stats_stats
############################# estimated stats #############################
#fi
...
cd ../ && make clean && GEN=ninja ENABLE_QUERY_SPLIT=1 ENABLE_CROSS_PRODUCT_REWRITE=1 VERBOSE=1 make > -a compile.log && cd measure && bash ./hyperfine_in_mem_job.sh query_split rsj
