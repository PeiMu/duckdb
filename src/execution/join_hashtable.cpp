...
static idx_t ProbeForPointers(JoinHashTable::ProbeState &state, JoinHashTable &ht, ht_entry_t *entries,
	...
	if (state.prefetch_enabled) {
		if (has_row_sel) {
			return ProbeForPointersInternal<USE_SALTS, true, true>(state, ht, entries, hashes_v, pointers_result_v, row_sel, count);
		} else {
			return ProbeForPointersInternal<USE_SALTS, false, true>(state, ht, entries, hashes_v, pointers_result_v, row_sel, count);
		}
	} else {
		if (has_row_sel) {
			return ProbeForPointersInternal<USE_SALTS, true, false>(state, ht, entries, hashes_v, pointers_result_v, row_sel, count);
		} else {
			return ProbeForPointersInternal<USE_SALTS, false, false>(state, ht, entries, hashes_v, pointers_result_v, row_sel, count);
		}
	}
}
...
idx_t ScanStructure::ResolvePredicates(DataChunk &keys, SelectionVector &match_sel, SelectionVector *no_match_sel) {
	...
	if (ht.needs_chain_matcher) {
		...
		result_count =
		    matcher->Match(keys, key_state.vector_data, match_sel, this->count, *ht.layout_ptr, pointers,
...
void JoinHashTable::PopulateAQPJITView(AQPJoinHTView &view) const {
	...
	view.bf_bitmask = 0;
	view.has_row_validity = (layout_ptr->GetDataOffset() == 0) ? 0ull : 1ull;
}
