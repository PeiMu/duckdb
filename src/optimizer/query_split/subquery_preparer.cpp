...
shared_ptr<PreparedStatementData> SubqueryPreparer::AdaptSelect(shared_ptr<PreparedStatementData> original_stmt_data,
	...
	auto subquery_stmt = make_shared_ptr<PreparedStatementData>(original_stmt_data->statement_type);
...
bool SubqueryPreparer::BlockUsed(const std::unordered_set<idx_t> &left_cond_table_index,
...
void SubqueryPreparer::ExplainAnalyzeSubQuery(ClientContextLock &lock,
	...
	auto explain_stmt_data = make_shared_ptr<PreparedStatementData>(StatementType::EXPLAIN_STATEMENT);
	auto explain_stmt =
	...
	explain_stmt_data->physical_plan = explain_physical_planner.Plan(std::move(explain_sub_plan));
	auto explain_prepared_stmt = make_uniq<PreparedStatement>(context.shared_from_this(), std::move(explain_stmt_data),
	                                                          statement_query, named_param_map);
...
SubqueryPreparer::CheckTableUsage(LogicalOperator *current_join_pointer, std::unordered_set<idx_t> &left_cond_table_index,
                                  std::unordered_map<idx_t, unique_ptr<LogicalOperator>> &table_blocks,
                                  std::deque<idx_t> &table_blocks_key_order,
                                  std::queue<unique_ptr<LogicalOperator>> &unused_blocks) {
	auto &current_join = current_join_pointer->Cast<LogicalComparisonJoin>();
