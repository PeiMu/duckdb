...
public:
	...
	// Save/restore for speculative peek (PeekNextSubquery).
	std::set<TableExpr> GetProjExprs() const { return proj_exprs; }
	void SetProjExprs(std::set<TableExpr> exprs) { proj_exprs = std::move(exprs); }
	int GetDataChunkSplitIndex() const { return data_chunk_split_index; }
	void SetDataChunkSplitIndex(int idx) { data_chunk_split_index = idx; }
...
private:
	...
	bool BlockUsed(const std::unordered_set<idx_t> &left_cond_table_index, const unique_ptr<LogicalOperator> &op);
