...
unique_ptr<ColumnDataCollection> RunLogicalPlanWithExecuteRow(Connection &conn,
                                                              unique_ptr<LogicalOperator> logical_plan,
                                                              const PlanMetadata &plan_meta,
                                                              bool print_physical_plan = false) {
	// Resolve types
	...
	if (print_physical_plan) {
		Printer::Print("=== Physical Plan for " + plan_meta.version + " split " +
		               std::to_string(plan_meta.split_index) + " ===");
		physical_plan->Print();
	}

	// Create PreparedStatementData with the pre-built plan
	auto prepared_data = make_shared<PreparedStatementData>(StatementType::SELECT_STATEMENT);
...
unique_ptr<QueryResult> RunLogicalPlan(Connection &conn, unique_ptr<LogicalOperator> logical_plan,
	...
	// Create PreparedStatementData with the pre-built plan
	auto prepared = make_shared<PreparedStatementData>(StatementType::SELECT_STATEMENT);
